        ...
        <div class="network-table-scroll">
          <table class="network-table" id="network-table-${type}-${code}">
            <thead><tr>
              ...
              ${isDist?'<th>Latest Tender Date</th><th>Competitors/Suppliers Carried</th><th>Priority</th><th>Contact</th>':'<th>Institution</th><th>Priority</th><th>Contact</th>'}
            </tr></thead>
            <tbody>
              ${rows.map(r=>{
                const priority=String(r._extra||'—');
                const search=[r.name,r.relevance,r.specialty,r.institution,r.contact,r.extra,r.latest_tender_date,r.competitors_carried].filter(Boolean).join(' ').toLowerCase();
                return `<tr data-search="${search}">
                  ...
                  ${isDist
                    ? `<td><div class="network-table-main">${r.latest_tender_date||'Not available'}</div></td><td><div class="network-table-main">${r.competitors_carried||'Not available'}</div></td><td><span class="network-priority ${networkPriorityClass(priority)}">${priority}</span></td><td><div class="network-contact-cell">${r.contact||'—'}</div></td>`
              ...
              }).join('') || `<tr><td colspan="${isDist?7:6}" class="network-empty">No records available for this country.</td></tr>`}
            </tbody>
          </table>
        </div>
