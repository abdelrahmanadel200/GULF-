...
<div class="dash">
  ...
  (function () {
    ...
    const countries = Object.keys(HA_DATA);
    ...
    function makeIcon(priority, rank) {
      ...
      var svg = '<svg xmlns="http://www.w3.org/2000/svg" width="'+(size*2)+'" height="'+(size*2)+'" viewBox="0 0 100 100">'
        + '<circle cx="50" cy="50" r="38" fill="' + color + '" opacity="0.25"/>'
    ...
    function loadCountry(c) {
      ...
      d.areas.forEach(function(a) {
        ...
        var popup = '<div style="background:#0d2145;border:1px solid #2563eb;border-radius:10px;padding:10px 14px;min-width:170px;font-family:Segoe UI,sans-serif;">'
          ...
          + '</div>';
        m.bindPopup(popup, { className: 'ha-popup', maxWidth: 220 });
        m.addTo(haMap);
        haMarkers.push(m);
      });
    }

    // Init map when Hot Areas tab is first clicked
    var haInited = false;
    var haObserver = new MutationObserver(function() {
      var page = document.getElementById("page-hotareas");
      if (page && page.classList.contains("active") && !haInited) {
        haInited = true;
        haMap = L.map("haMap", { zoomControl: true, attributionControl: false });
        L.tileLayer("https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png", {
          maxZoom: 19
        }).addTo(haMap);
        // Remove default popup styles conflict
        var style = document.createElement("style");
        style.textContent = ".leaflet-popup-content-wrapper{background:transparent!important;border:none!important;box-shadow:none!important;padding:0!important;} .leaflet-popup-content{margin:0!important;} .leaflet-popup-tip{display:none!important;}";
    ...
    // Also handle navigate function override to trigger observer
    var _origNavigate = window.navigate;
    window.navigate = function(el, pageId) {
      _origNavigate(el, pageId);
      if (pageId === "hotareas" && !haInited) {
        setTimeout(function(){ haObserver.disconnect(); var page = document.getElementById("page-hotareas"); if(page){ var ev = new MutationRecord(); haObserver.observe(page,{attributes:true,attributeFilter:["class"]}); }}, 50);
      }
      if (pageId === "hotareas" && haInited && haMap) {
        setTimeout(function(){ haMap.invalidateSize(); }, 200);
      }
    };
  })();
...
<script>
  ...
  function initHaMap() {
    haInited = true;
    haMap = L.map('haMap', {zoomControl:true, attributionControl:false});
    L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png', {maxZoom:19}).addTo(haMap);
    loadCountry(haActive);
  }
