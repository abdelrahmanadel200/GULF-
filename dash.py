...
WORKBOOK_CANDIDATES = [
    "Amecath Dash.xlsx",
    "Amecath Dash - corrected.xlsx",
    "Amecath_Dash.xlsx",
]
_BASE_DIR = Path(__file__).resolve().parent
WORKBOOK_PATH = next((_BASE_DIR / name for name in WORKBOOK_CANDIDATES if (_BASE_DIR / name).exists()), None)
if WORKBOOK_PATH is None:
    # Also accept a single Excel workbook with an Amecath/Dash-like filename.
    _xlsx = sorted(_BASE_DIR.glob("*.xlsx"))
    WORKBOOK_PATH = next((p for p in _xlsx if "amecath" in p.name.lower() or "dash" in p.name.lower()), None)
if WORKBOOK_PATH is None:
    st.error("Workbook not found. Put 'Amecath Dash.xlsx' next to this dashboard file.")
    st.stop()

_wb = openpyxl.load_workbook(WORKBOOK_PATH, data_only=True)

def _country_code(label):
    s = re.sub(r"^[^\w]+", "", str(label or "")).strip()
...
for row in _wb["Financials_Tenders"].iter_rows(min_row=2,max_row=21,values_only=True):
    if row[0]:
        ...
        _tenders.append({"id":row[0],"country":row[1],"display":row[1],"country_code":_country_code(row[1]),"name":row[2],"ref":str(row[3]),
...
COUNTRY_CODES = {"Saudi Arabia":"sa","UAE":"ae","Qatar":"qa","Kuwait":"kw","Oman":"om","Bahrain":"bh","Jordan":"jo","Lebanon":"lb","Iraq":"iq"}
def _clean_cell(v): return None if v is None else str(v).strip()
def parse_distributors(ws):
    out={}; current=None; headers=False; lookup={k.upper():k for k in COUNTRY_CODES}
    for row in ws.iter_rows(values_only=True):
        a=_clean_cell(row[0]); b=_clean_cell(row[1])
        if a and not b and a!='#':
            current=COUNTRY_CODES.get(lookup.get(a.upper(),a));
            if current: out[current]=[]
            headers=False; continue
        if a=='#': headers=True; continue
        if current and headers and row[0] is not None:
            out[current].append({"num":row[0],"name":_clean_cell(row[1]) or "—","relevance":_clean_cell(row[2]) or "—","contact":_clean_cell(row[3]) or "—","website":_clean_cell(row[4]),"latest_tender_date":_clean_cell(row[5]) or "Not available","competitors_carried":_clean_cell(row[6]) or "Not available","priority":_clean_cell(row[7]) or "—"})
    return out
def parse_regulatory(ws):
    out={}; current=None; item=None; lookup={k.lower():k for k in COUNTRY_CODES}
    for row in ws.iter_rows(values_only=True):
        a=_clean_cell(row[0]); b=_clean_cell(row[1])
        if a and not b and a.lower()!='item':
            current=COUNTRY_CODES.get(lookup.get(a.lower(),a)); item=None
            if current: out[current]={"country":a,"items":[]}
            continue
        if not current: continue
        if a and a.lower()!='item':
            item={"item":a,"details":[]}; out[current]["items"].append(item)
            if b: item["details"].append(b)
        elif item and b: item["details"].append(b)
    return out
def parse_procurement(ws):
    out=[]
    for row in ws.iter_rows(min_row=2,values_only=True):
        if not row[1]: continue
        country=_clean_cell(row[1]); code=next((c for n,c in COUNTRY_CODES.items() if n.lower() in country.lower()),None)
        out.append({"num":row[0],"country":country,"country_code":code,"primary_body":_clean_cell(row[2]) or "—","secondary_buyers":_clean_cell(row[3]) or "—","notes":_clean_cell(row[4]) or "—"})
    return out
DISTRIBUTOR_DATA=parse_distributors(_wb["Distributors"])
REGULATORY_DATA=parse_regulatory(_wb["Regulatory "])
PROCUREMENT_DATA=parse_procurement(_wb["procurement body"])

# Forecast detail
_fw=_wb["Forecast_Data"]
_FORECAST_COUNTRIES=[]
for row in _fw.iter_rows(min_row=13,max_row=21,values_only=True):
    if row[0]: _FORECAST_COUNTRIES.append({"country":_clean_cell(row[0]),"country_code":COUNTRY_CODES.get(_clean_cell(row[0])),"hd":row[1],"pd":row[2],"unit_growth":row[3],"demand_2026":row[4],"demand_2027":row[5],"demand_2028":row[6],"std_asp":row[7],"mid_asp":row[8],"tunneled_asp":row[9],"blended_asp":row[10],"share_2026":row[11],"units_2026":row[12],"revenue_2026":row[13],"revenue_2027":row[14],"revenue_2028":row[15]})
FORECAST_DATA={"countries":_FORECAST_COUNTRIES,"product_mix":{"std":_fw["A27"].value,"mid_term":_fw["B27"].value,"tunneled":_fw["C27"].value},"methodology":[_clean_cell(_fw.cell(r,1).value) for r in range(31,40) if _fw.cell(r,1).value]}

_EXHIBITIONS=[]
for row in _wb["Medical Exhibitions"].iter_rows(min_row=2,values_only=True):
    if row[0]: _EXHIBITIONS.append({"num":row[0],"name":_clean_cell(row[1]),"dates":_clean_cell(row[2]),"location":_clean_cell(row[3]),"focus":_clean_cell(row[4]),"audience":_clean_cell(row[5]),"website":_clean_cell(row[6])})
EXHIBITIONS_DATA=_EXHIBITIONS

WORKBOOK_DATA={"macro":_macro,"competitors":_comp,"tenders":_tenders,"hotAreas":_hot,
               "ourASP":_our_asp,"competitorASP":_comp_asp,"distributors":DISTRIBUTOR_DATA,"regulatory":REGULATORY_DATA,"procurement":PROCUREMENT_DATA,"forecast":FORECAST_DATA,"exhibitions":EXHIBITIONS_DATA}
...
.reg-country-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;margin:0 16px 20px}.reg-country-card,.proc-country-card{background:#0f1f3d;border:1px solid #1e3d7a;border-radius:14px;overflow:hidden}.reg-country-head,.proc-country-head{padding:15px 17px;background:#10264a;border-bottom:1px solid #1e3d7a;cursor:pointer}.reg-country-head strong,.proc-country-head strong{color:#e8edf5;font-size:13px}.reg-country-meta{color:#6a85b0;font-size:10px;margin-top:4px}.reg-country-body{display:none;padding:14px}.reg-country-card.open .reg-country-body{display:block}.reg-item{border:1px solid #183258;border-radius:10px;background:#0b1628;margin-bottom:8px;overflow:hidden}.reg-item-title{padding:10px 12px;color:#60a5fa;font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.5px}.reg-item-details{padding:0 12px 11px;color:#c8d8f0;font-size:11px;line-height:1.55}.reg-item-details ul{margin:0;padding-left:18px}.proc-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;margin:0 16px 20px}.proc-country-body{padding:13px 15px}.proc-label{font-size:8px;color:#6a85b0;text-transform:uppercase;letter-spacing:.8px;font-weight:800;margin-bottom:4px}.proc-value{font-size:11px;color:#dce6f4;line-height:1.5;margin-bottom:11px}.proc-tender-wrap{margin-top:10px;border-top:1px solid #183258;padding-top:10px;overflow-x:auto}.proc-mini-table{width:100%;min-width:620px;border-collapse:collapse;font-size:10px}.proc-mini-table th{padding:8px;background:#071326;color:#6a85b0;text-align:left;text-transform:uppercase;font-size:8px}.proc-mini-table td{padding:8px;color:#c8d8f0;border-top:1px solid #152c4d}@media(max-width:800px){.reg-country-grid,.proc-grid{grid-template-columns:1fr}}
.network-table-wrap{margin:0 20px 20px;background:#0b1830;border:1px solid #1b3a67;border-radius:16px;overflow:hidden}.network-table-scroll{width:100%;overflow-x:auto}.network-table{width:100%;min-width:1180px;border-collapse:collapse}.network-table thead th{padding:11px 13px;background:#071326;color:#7893b9;border-bottom:1px solid #1b3a67;text-align:left;font-size:9px;font-weight:800;text-transform:uppercase;letter-spacing:.7px;white-space:nowrap}.network-table tbody td{padding:13px;color:#dce6f4;border-bottom:1px solid #152c4d;vertical-align:middle;font-size:10px}.network-table tbody tr:hover{background:rgba(37,99,235,.07)}.network-table-num{width:45px;color:var(--net-accent)!important;font-weight:800;text-align:center}.network-table-name{color:#fff;font-size:11px;font-weight:800;min-width:150px}.network-table-main{color:#c7d5e8;line-height:1.5;min-width:170px}.network-contact-cell{color:#9eb2cc;line-height:1.5;min-width:160px;word-break:break-word}.network-empty{text-align:center!important;padding:35px!important;color:#607a9f!important}
</style>
...
<div class="sidebar">
  ...
  <div class="nav-item" data-page="procurement" role="button" tabindex="0" onclick="return window.sidebarNavigate('procurement');"><span class="nav-icon">🏛️</span><span>Procurement Body</span></div>
...
<div class="page" id="page-tenders">
  <div style="padding:0 16px 32px;">
    ...
    <div id="tndr-section-active">
      <div class="tndr-kpis">
        <div class="tndr-kpi"><div class="tndr-kpi-label">Workbook Tenders</div><div class="tndr-kpi-value" id="tndr-kpi-total">20</div></div>
        ...
        <div class="tndr-kpi"><div class="tndr-kpi-label">Critical Priority</div><div class="tndr-kpi-value" style="color:#f59e0b;" id="tndr-kpi-critical">0</div></div>
        <div class="tndr-kpi"><div class="tndr-kpi-label">Open / Active</div><div class="tndr-kpi-value" style="color:#3b82f6;" id="tndr-kpi-active">0</div></div>
        <div class="tndr-kpi"><div class="tndr-kpi-label">Closed</div><div class="tndr-kpi-value" style="color:#34d399;" id="tndr-kpi-closed">0</div></div>
      </div>
...
<div class="page" id="page-procurement"><div class="section-header"><span style="font-size:16px">🏛️</span><span class="section-title">Procurement Bodies & Tender Linkage</span></div><div class="proc-grid" id="procurement-grid"></div></div>
<!-- REGULATORY -->
<div class="page" id="page-regulatory"><div class="section-header"><span style="font-size:16px">📜</span><span class="section-title">Regulatory & Registration Intelligence</span></div><div class="reg-country-grid" id="reg-country-grid"></div></div>
...
const networkData = {"distributors":{"sa":[{"num":1,"name":"AMHSCO – Arabian Medical Hospital Supply","relevance":"Very High — medical devices, life-sciences, specifically has a renal division","contact":"☎️ +966 11 462 1188 · ✉️ sales@amhsco.com","website":"AMHSCO","latest_tender_date":"Aug 2026 (NUPCO NDP0788/26 dialysis framework) nupco","competitors_carried":"Fresenius KSA, B. Braun KSA, multiple HD catheter brands","priority":"🔴 5/5"},{"num":2,"name":"AlwanMed","relevance":"Very High — licensed medical-device distributor; serves major government/private hospitals; tender submissions","contact":"Contact through website","website":"AlwanMed","latest_tender_date":"Jul 2026 (NUPCO medical device tenders)","competitors_carried":"Medtronic KSA, BD KSA, vascular access brands","priority":"🔴 5/5"},{"num":3,"name":"Aman Medical","relevance":"Very High — explicitly supplies dialysis systems and medical devices","contact":"☎️ +966 54 882 1508 · ✉️ info@amanmedical.com","website":"Aman Medical","latest_tender_date":"Aug 2026 (NUPCO dialysis consumables) nupco","competitors_carried":"Fresenius, Baxter, B. Braun dialysis portfolio","priority":"🔴 5/5"},{"num":4,"name":"FUMEDCO / MNAF3 Arabia","relevance":"High — medical equipment, devices & disposables; supplies large government hospitals","contact":"☎️ +966 11 400 3493 · ✉️ info@mnaf3arabia.com","website":"FUMEDCO","latest_tender_date":"Jun 2026 (NUPCO medical device tenders)","competitors_carried":"Multiple international HD catheter brands","priority":"🟠 4/5"},{"num":5,"name":"House of Rays Medical","relevance":"High — 350+ healthcare clients; medical consumables; nationwide coverage","contact":"Website / WhatsApp","website":"House of Rays Medical","latest_tender_date":"Jul 2026 (NUPCO open framework for dialysis supplies) nupco","competitors_carried":"Authorized distributor for global medical manufacturers since 2005","priority":"🟠 4/5"},{"num":6,"name":"Nipras AlSalhiya Medical","relevance":"High — medical equipment/devices/accessories; branches Dammam, Riyadh, Jeddah, Tabuk, Khamis","contact":"Website contact","website":"Nipras Medical","latest_tender_date":"Aug 2026 (NUPCO dialysis framework) nupco","competitors_carried":"Multiple HD catheter brands; vascular access devices","priority":"🟠 4/5"},{"num":7,"name":"Jama Medical","relevance":"High — nationwide logistics; Riyadh/Jeddah/Dammam/Qassim facilities","contact":"Website contact","website":"Jama Medical","latest_tender_date":"Jul 2026 (NUPCO medical equipment tenders)","competitors_carried":"Comprehensive medical equipment portfolio; HD catheters","priority":"🟠 4/5"},{"num":8,"name":"Watan Medical Company","relevance":"High — medical devices, supplies & equipment across KSA","contact":"☎️ +966 13 833 3606 · ✉️ info@watanmedical.com","website":"Watan Medical","latest_tender_date":"Jun 2026 (NUPCO dialysis consumables) nupco","competitors_carried":"Fresenius KSA, multiple dialysis consumable brands","priority":"🟠 4/5"},{"num":9,"name":"Healthcare Systems Saudi","relevance":"Medium–High — medical consumables, hospital supplies; serves MOH, military, National Guard and private hospitals","contact":"☎️ +966 92 000 4438 · ✉️ sales@hs-saudi.com","website":"Healthcare Systems Saudi","latest_tender_date":"Aug 2026 (NUPCO medical device tenders)","competitors_carried":"Multiple HD catheter brands; hospital supplies","priority":"🟡 3/5"},{"num":10,"name":"Raqwani Medicals","relevance":"Medium–High — medical devices, equipment, surgical supplies and hospital products","contact":"☎️ +966 56 393 3574 / +966 50 124 3758 · ✉️ info@raqwanimedicals.com","website":"Raqwani Medicals","latest_tender_date":"Jul 2026 (NUPCO framework participation) nupco","competitors_carried":"Multiple international HD catheter brands","priority":"🟡 3/5"}],"ae":[{"num":1,"name":"GulfDrug LLC","relevance":"Very High — one of UAE’s largest healthcare distributors; explicit dialysis-equipment portfolio and long-standing hospital supply relationships.","contact":"☎️ +971 4 501 4000 · ✉️ info@gulfdrug.com","website":"GulfDrug","latest_tender_date":"Aug 2026 (MOHAP/SEHA consumables frameworks; GulfDrug routinely bids on national dialysis-consumable tenders)","competitors_carried":"Fresenius Medical Care (dialysis systems & consumables), B. Braun, multiple international HD catheter brands","priority":"🔴 5/5"},{"num":2,"name":"TTSA Medical Group FZCO","relevance":"Very High — official UAE distributor for B. Braun and Nipro dialysis systems/consumables; strong renal focus.","contact":"✉️ info@ttsa-group.com (Dubai Silicon Oasis, IFZA)","website":"TTSA Medical","latest_tender_date":"Jul 2026 (SEHA/MOHAP dialysis-consumable frameworks)","competitors_carried":"B. Braun, Nipro; full dialysis consumables line (tubing, needles, catheters, concentrates)","priority":"🔴 5/5"},{"num":3,"name":"One Health (PureHealth)","relevance":"Very High — authorized distributor for top global medical-device brands; dedicated renal-care division serving 300+ providers.","contact":"Contact via website (PureHealth/One Health portal)","website":"One Health","latest_tender_date":"Aug 2026 (SEHA/MOHAP device frameworks)","competitors_carried":"Medtronic, Siemens Healthineers, GE HealthCare, Philips; renal-care portfolios from major OEMs","priority":"🔴 5/5"},{"num":4,"name":"Zahrawi Group (Al Zahrawi Medical Supplies)","relevance":"High — GCC-wide medical/surgical distributor with UAE HQ; lists catheters and dialysis solutions in portfolio.","contact":"Contact via website (Dubai HQ)","website":"Zahrawi Group","latest_tender_date":"Jun 2026 (MOHAP/SEHA surgical & consumable tenders)","competitors_carried":"Multiple international catheter brands; surgical/ICU consumables; dialysis solutions","priority":"🟠 4/5"},{"num":5,"name":"Emirates & World Medical Supplies (EWMS)","relevance":"High — broad medical-disposables and equipment distributor; serves hospitals, authorities, and defense; strong supply-chain capability.","contact":"☎️ +971 4 447-0098 · ✉️ support@ewms.ae","website":"EWMS","latest_tender_date":"Jul 2026 (MOHAP/SEHA consumables frameworks)","competitors_carried":"Infection-control, ICU, anesthesia/nerve-block (Pajunk), multi-brand disposables; hospital supply networks","priority":"🟠 4/5"},{"num":6,"name":"Medeon Medical Equipment Trading LLC","relevance":"High — explicit dialysis-equipment supplier in UAE; lists hemodialysis catheters among core products.","contact":"☎️ +971 4 572 2034 / +971 52 994 5833 · ✉️ info@medeonmed.com","website":"Medeon","latest_tender_date":"Jun 2026 (private-hospital and clinic dialysis procurements)","competitors_carried":"Multiple HD catheter brands; dialysis machines, tubing, dialyzers, concentrates","priority":"🟠 4/5"},{"num":7,"name":"Winray Medical Equipment Trading LLC","relevance":"High — dialysis-equipment supplier in Dubai; serves hospitals/clinics with dialysis machines and accessories.","contact":"☎️ +971 4 282 3307 / +971 52 352 2291 · ✉️ info@winraymed.com","website":"Winray","latest_tender_date":"Jun 2026 (private-sector dialysis procurements)","competitors_carried":"Multiple HD catheter brands; dialysis machines and consumables","priority":"🟠 4/5"},{"num":8,"name":"Majestic Medical","relevance":"High — vascular-access reseller with dedicated long-term and short-term hemodialysis catheters.","contact":"Contact via website","website":"Majestic Medical","latest_tender_date":"Jul 2026 (private hospital vascular-access tenders)","competitors_carried":"International vascular-access brands; HD catheters (long/short term)","priority":"🟠 4/5"},{"num":9,"name":"Al-Futtaim Health (HealthHub)","relevance":"Medium–High — integrated healthcare operator with clinic network; procures consumables and devices across UAE.","contact":"☎️ +971 4 596 7000 · ✉️ info.healthhub@alfuttaim.com","website":"Al-Futtaim Health","latest_tender_date":"Aug 2026 (internal framework renewals for clinic consumables)","competitors_carried":"Multi-brand medical consumables; works with global OEMs via group procurement","priority":"🟡 3/5"},{"num":10,"name":"Royal Care Medical Equipment Trading LLC","relevance":"Medium–High — supplier of disposable medical products and surgical supplies to hospitals/clinics across UAE.","contact":"☎️ +971 52 641 9796 · ✉️ sapana@royalcaremedicalsuae.com","website":"Royal Care","latest_tender_date":"Jun 2026 (private hospital consumable tenders)","competitors_carried":"Romsons, AP Medical; catheters & tubing, IV sets, surgical gloves, PPE","priority":"🟡 3/5"}],"qa":[{"num":1,"name":"Fayn Al Tbyh / Fayn Medical","relevance":"Vascular access + medical consumables; explicitly offers vascular-access products and partnership/distribution services","contact":"☎️ +974 4491 9296 / +974 4431 0911 · ✉️ info@fayn.qa","website":"Fayn Medical","latest_tender_date":"Jun 2026 (private-hospital consumable tenders)","competitors_carried":"Multi-brand vascular-access and dialysis consumables; international catheter OEMs","priority":"🔴 5/5"},{"num":2,"name":"Barzan Medical Supplies","relevance":"Major Qatar healthcare distributor; medical equipment, hospital disposables & consumables; strong Hamad Medical Corp ties","contact":"☎️ +974 4441 0270 · ✉️ info@barzanmedical.com","website":"Barzan Medical","latest_tender_date":"Jul 2026 (HMC/MOH consumable frameworks)","competitors_carried":"Global OEMs across surgical, ICU, and vascular access; multi-brand catheters","priority":"🔴 5/5"},{"num":3,"name":"Gulf Engineering & Technical Services (GENTECH)","relevance":"One of Qatar’s largest biomedical/medical-equipment groups; supplies hospitals/clinics nationwide","contact":"☎️ +974 4486 8100 · ✉️ gentech@gentechqa.com","website":"GENTECH","latest_tender_date":"Aug 2026 (MOHAP/SEHA-style consumable frameworks)","competitors_carried":"Multiple international HD catheter brands; dialysis machines and consumables","priority":"🔴 5/5"},{"num":4,"name":"Care Medical Trading","relevance":"Established medical-equipment distributor serving healthcare, diagnostics & life sciences","contact":"Website/contact form","website":"Care Medical","latest_tender_date":"Jun 2026 (private-hospital procurements)","competitors_carried":"International medical-device brands; catheters and disposables","priority":"🔴 5/5"},{"num":5,"name":"Universal Trade Line (UTL)","relevance":"20+ years distributing medical equipment, consumables & laboratory products to government/private sectors","contact":"Website/contact","website":"UTL Qatar","latest_tender_date":"Jul 2026 (MOH consumable frameworks)","competitors_carried":"Global manufacturers; vascular access and catheters","priority":"🔴 5/5"},{"num":6,"name":"Origin Trading & Contracting WLL","relevance":"Medical equipment + consumables + procurement/logistics; 13+ years","contact":"☎️ +974 4002 0246 · ✉️ info@originqatar.com","website":"Origin Qatar","latest_tender_date":"Jun 2026 (private-sector consumable tenders)","competitors_carried":"Multi-brand disposables; catheters, IV sets, surgical supplies","priority":"🟠 4/5"},{"num":7,"name":"Khalid Scientific Company","relevance":"Long-standing medical-equipment supplier with dialysis and hospital-supply portfolios","contact":"☎️ +974 4441 7371 / +974 4432 5198","website":"Khalid Scientific","latest_tender_date":"Jun 2026 (private-hospital dialysis procurements)","competitors_carried":"Multiple HD catheter brands; dialysis consumables and machines","priority":"🟠 4/5"},{"num":8,"name":"Ibn Al Haytham Centre","relevance":"Medical devices/equipment + consumables + surgical instruments","contact":"☎️ +974 4431 2283 · ✉️ sales@ibncentre.com","website":"Ibn Al Haytham","latest_tender_date":"Jun 2026 (private-hospital consumable tenders)","competitors_carried":"International medical-equipment brands; catheters and disposables","priority":"🟠 4/5"},{"num":9,"name":"Gulfmed Medical Supplies","relevance":"Medical-supply company in Doha; broad consumables range","contact":"☎️ +974 4486 6216","website":"—","latest_tender_date":"Jun 2026 (private-sector consumable tenders)","competitors_carried":"Multi-brand medical consumables; catheters and surgical supplies","priority":"🟠 4/5"},{"num":10,"name":"Novel Medical Solutions W.L.L.","relevance":"Healthcare supply & distribution company; wholesale medical supplies","contact":"☎️ +974 4467 5151 · ✉️ info@novelmedsolution.com","website":"Novel Medical","latest_tender_date":"Jul 2026 (MOH consumable frameworks)","competitors_carried":"International medical consumables; vascular access and catheters","priority":"🟠 4/5"}],"kw":[{"num":1,"name":"Advanced Technology Company (ATC)","relevance":"~90% market share in dialysis systems; strong government-hospital relationships","contact":"☎️ +965 2224 7444 · ✉️ info@atc.com.kw","website":"ATC Kuwait","latest_tender_date":"Aug 2026 (MOH dialysis-system/consumable frameworks)","competitors_carried":"Fresenius, Baxter, B. Braun dialysis portfolios; HD catheters","priority":"🔴 5/5"},{"num":2,"name":"Arabi Medical & Scientific Equipment","relevance":"Major healthcare supplier; explicitly supplies dialysis equipment + consumables","contact":"Arabi Holding contact","website":"Arabi Holding","latest_tender_date":"Jul 2026 (MOH dialysis/consumable frameworks)","competitors_carried":"Global dialysis and medical-device OEMs; catheters and consumables","priority":"🔴 5/5"},{"num":3,"name":"DMC Trading Co.","relevance":"25+ years; supplies MOH, Ministry of Defense & private hospitals","contact":"☎️ +965 6515 0700 · ✉️ info@dmc-kw.com","website":"DMC Kuwait","latest_tender_date":"Jul 2026 (MOH surgical/consumable tenders)","competitors_carried":"International medical manufacturers; catheters and disposables","priority":"🔴 5/5"},{"num":4,"name":"United Medical Commodities (UMC)","relevance":"20+ years; represents international medical manufacturers","contact":"☎️ +965 2245 0815/6 · ✉️ info@medcom.com.kw","website":"UMC Kuwait","latest_tender_date":"Jun 2026 (private-hospital dialysis procurements)","competitors_carried":"Multiple international HD catheter brands; vascular access","priority":"🔴 5/5"},{"num":5,"name":"Medical Means Co. / Al Redwan Group","relevance":"Major GCC dialysis/medical-supply organization","contact":"Regional contact","website":"Redwan Medical","latest_tender_date":"Jun 2026 (private-hospital dialysis procurements)","competitors_carried":"Fresenius, Baxter, B. Braun dialysis portfolios; HD catheters","priority":"🔴 5/5"},{"num":6,"name":"Medvision for Medical Services","relevance":"Top-5 medical distributor in Kuwait; dedicated disposable division supplying hospitals daily","contact":"☎️ +965 2202 2228 · ✉️ info@medvision-kw.com","website":"Medvision Kuwait","latest_tender_date":"Jul 2026 (MOH surgical/consumable tenders)","competitors_carried":"Multiple international HD catheter brands; broad disposables portfolio","priority":"🔴 5/5"},{"num":7,"name":"Leader Medical Company","relevance":"Medical equipment + supplies; hospital/clinic distribution","contact":"☎️ +965 2246 1967","website":"—","latest_tender_date":"Jun 2026 (private-sector consumable tenders)","competitors_carried":"Multi-brand catheters and surgical consumables","priority":"🟠 4/5"},{"num":8,"name":"Warba Medical Supplies Co","relevance":"Medical supplies; broad consumables portfolio","contact":"☎️ +965 2232 3850","website":"—","latest_tender_date":"Jun 2026 (private-hospital consumable tenders)","competitors_carried":"International consumables; catheters and IV sets","priority":"🟠 4/5"},{"num":9,"name":"Ahmed Company for Wholesale","relevance":"30+ years; clinical disposables, surgical tools & medical consumables; handles tenders","contact":"Sales team / website","website":"Ahmed Company","latest_tender_date":"Jul 2026 (MOH consumable frameworks)","competitors_carried":"Clinical disposables, surgical tools; multi-brand catheters","priority":"🟠 4/5"},{"num":10,"name":"New Star Company WLL","relevance":"High-tech medical-equipment distributor with exclusive international partnerships","contact":"☎️ +965 5515 0547 · ✉️ info@starmedicalkw.com","website":"New Star Kuwait","latest_tender_date":"Jun 2026 (private-hospital dialysis procurements)","competitors_carried":"Global manufacturers; vascular access and catheters","priority":"🟠 4/5"}],"om":[{"num":1,"name":"Taiba Medserv (Taiba Medical Services)","relevance":"Leading medical-equipment distributor; strong MOH relationships and service capability","contact":"☎️ +968 2459 3395 / +968 2459 6695 · ✉️ medical@omanmed.com","website":"Taiba Medserv","latest_tender_date":"Aug 2026 (MOH renal-dialysis device/consumable tenders)","competitors_carried":"Fresenius, B. Braun, multiple HD catheter brands","priority":"🔴 5/5"},{"num":2,"name":"Oman Medical Supplies & Services (OMANMED / Suhail Bahwan)","relevance":"Major medical/laboratory distributor; nationwide hospital coverage","contact":"☎️ +968 2465 0750 / +968 2465 9778 · ✉️ bhc@suhailbahwangroup.com","website":"Suhail Bahwan Medical","latest_tender_date":"Jul 2026 (MOH consumable frameworks)","competitors_carried":"Global OEMs; dialysis consumables and catheters","priority":"🔴 5/5"},{"num":3,"name":"Medical & Scientific Supplies LLC","relevance":"Imports/markets hospital equipment, surgical products, pharmaceuticals & consumables","contact":"☎️ +968 2449 7844","website":"—","latest_tender_date":"Jul 2026 (MOH device/consumable tenders)","competitors_carried":"International medical-device brands; catheters and disposables","priority":"🔴 5/5"},{"num":4,"name":"Al Farsi Medical Supplies (AFMS)","relevance":"Established importer/distributor; surgical consumables + medical equipment","contact":"☎️ +968 2448 5625 · WhatsApp +968 9225 8225","website":"—","latest_tender_date":"Jun 2026 (private-hospital consumable tenders)","competitors_carried":"Multi-brand surgical consumables; catheters","priority":"🔴 5/5"},{"num":5,"name":"Niemath Al Noor Trading LLC (NieMed)","relevance":"Nationwide medical equipment + consumables; hospital/clinic supply","contact":"☎️ +968 7928 3733 / +968 7952 7382 · ✉️ info@niemathalnoor.com","website":"NieMed Oman","latest_tender_date":"Jun 2026 (private-hospital consumable tenders)","competitors_carried":"Global manufacturers; catheters and disposables","priority":"🔴 5/5"},{"num":6,"name":"MSTE LLC","relevance":"Medical devices, consumables & disposables; large Muscat warehouse","contact":"☎️ +968 2423 8417 · ✉️ info@msteoman.com","website":"MSTE Oman","latest_tender_date":"Jul 2026 (MOH consumable frameworks)","competitors_carried":"Multiple international HD catheter brands; hospital supplies","priority":"🔴 5/5"},{"num":7,"name":"Mazoon Medical Supplies","relevance":"Public/private healthcare supply; consumables including catheters","contact":"☎️ +968 9644 2500 · ✉️ info@mazoonmedical.com","website":"Mazoon Medical","latest_tender_date":"Jun 2026 (private-hospital consumable tenders)","competitors_carried":"Certified global manufacturers; catheters and surgical disposables","priority":"🟠 4/5"},{"num":8,"name":"Seha Medical Supplies","relevance":"Medical equipment; cardiac & vascular solutions","contact":"☎️ +968 2411 2944 · ✉️ info@seha.om","website":"Seha Oman","latest_tender_date":"Jun 2026 (private-hospital vascular-access tenders)","competitors_carried":"International cardiac/vascular brands; catheters","priority":"🟠 4/5"},{"num":9,"name":"MuscatMed / HUI Medical Supplies","relevance":"Importer/distributor of medical equipment & surgical disposables","contact":"☎️ +968 7909 8973 / +968 9257 5465","website":"—","latest_tender_date":"Jun 2026 (private-hospital consumable tenders)","competitors_carried":"Multi-brand medical equipment; catheters and disposables","priority":"🟠 4/5"},{"num":10,"name":"Advanced Medical Instruments Co. (AMICO)","relevance":"Medical equipment, medical supplies & consumables","contact":"Oman Yellow Pages / company contact","website":"—","latest_tender_date":"Jun 2026 (private-sector procurements)","competitors_carried":"International medical consumables; catheters","priority":"🟠 4/5"}],"jo":[{"num":1,"name":"Micromed Medical Supplies Co.","relevance":"One of Jordan's largest interventional/surgical/medical-product suppliers; 15+ years' experience","contact":"☎️ +962 6 553 3389 · ✉️ info@micromedjo.com","website":"Micromed Jordan","latest_tender_date":"Aug 2026 (MOH/UNRWA consumable frameworks)","competitors_carried":"Global corporations; interventional and vascular-access portfolios","priority":"🔴 5/5"},{"num":2,"name":"Greenland Medical","relevance":"Vascular Access + Interventional Radiology specifically listed","contact":"☎️ +962 6 515 6480 · +962 79 588 7422","website":"—","latest_tender_date":"Jul 2026 (private-hospital interventional tenders)","competitors_carried":"International vascular-access and IR brands; catheters","priority":"🔴 5/5"},{"num":3,"name":"Hijazi Medical Supplies (HMS)","relevance":"Catheters + vascular/interventional products","contact":"☎️ +962 6 515 4826 · ✉️ info@hijazibros.com","website":"Hijazi Bros","latest_tender_date":"Jun 2026 (private-hospital vascular-access tenders)","competitors_carried":"Multi-brand catheters; vascular/interventional products","priority":"🔴 5/5"},{"num":4,"name":"NAJD Medical","relevance":"Interventional products; supplies major public/private hospitals","contact":"☎️ +962 79 621 7161 · ✉️ elayan@najdmed.com","website":"Najd Medical","latest_tender_date":"Jul 2026 (private-hospital interventional tenders)","competitors_carried":"Well-known international brands; catheters and interventional devices","priority":"🔴 5/5"},{"num":5,"name":"RAMANA Medical Supplies","relevance":"Vascular + interventional radiology; distributor for international companies","contact":"Contact via website","website":"RAMANA Jordan","latest_tender_date":"Jun 2026 (private-hospital consumable tenders)","competitors_carried":"International companies; vascular and IR portfolios","priority":"🔴 5/5"},{"num":6,"name":"World Medical Supplies (WMS)","relevance":"Supplies Jordan MOH + Royal Medical Services","contact":"☎️ +962 79 914 0755","website":"—","latest_tender_date":"Jul 2026 (MOH consumable frameworks)","competitors_carried":"Multi-brand medical consumables; catheters","priority":"🔴 5/5"},{"num":7,"name":"United for Marketing","relevance":"Works directly with Jordan MOH and public/private hospitals","contact":"Website contact","website":"—","latest_tender_date":"Jul 2026 (MOH consumable frameworks)","competitors_carried":"Global medical-device brands; catheters and disposables","priority":"🔴 5/5"},{"num":8,"name":"Al-Ahlia Company","relevance":"Medical-device distributor since 1987; strong local infrastructure","contact":"☎️ +962 6 465 0951 · +962 77 736 8181 · ✉️ info@ahliamed.com","website":"Ahlia Medical","latest_tender_date":"Jun 2026 (private-hospital consumable tenders)","competitors_carried":"International medical-device brands; catheters","priority":"🟠 4/5"},{"num":9,"name":"Redwan Medical Group","relevance":"Regional distributor (KSA/UAE/Kuwait/Jordan) of dialysis equipment and hospital consumables","contact":"Contact via website","website":"Redwan Medical","latest_tender_date":"Jun 2026 (private-hospital dialysis procurements)","competitors_carried":"Fresenius, Baxter, B. Braun dialysis portfolios; HD catheters","priority":"🟠 4/5"},{"num":10,"name":"Surur Medical","relevance":"Medical equipment + surgical consumables; hospital/medical-center distribution","contact":"Website contact","website":"Surur Medical","latest_tender_date":"Jun 2026 (private-hospital consumable tenders)","competitors_carried":"Multi-brand surgical consumables; catheters","priority":"🟠 4/5"}],"lb":[{"num":1,"name":"Medical & Technical Services (MTS)","relevance":"Explicit dialysis-catheter portfolio; serves hospitals nationwide","contact":"☎️ +961 5 811 027 / +961 5 811 028","website":"MTS Lebanon","latest_tender_date":"Aug 2026 (private-hospital dialysis-consumable tenders)","competitors_carried":"Multiple international HD catheter brands; PICC and CVC lines","priority":"🔴 5/5"},{"num":2,"name":"MedTrust Solutions","relevance":"Vascular access + hemodialysis + endovascular","contact":"☎️ +961 3 293 893 · contact via website","website":"MedTrust","latest_tender_date":"Jul 2026 (private-hospital vascular-access tenders)","competitors_carried":"Global vascular-access and endovascular brands; catheters","priority":"🔴 5/5"},{"num":3,"name":"Fattal Group (Healthcare Division)","relevance":"Major healthcare distributor with aesthetic/medical-devices division; broad hospital network","contact":"☎️ +961 1 485 250 · ✉️ Elie.Moubarak@fattal.com.lb","website":"Fattal Healthcare","latest_tender_date":"Jul 2026 (private-hospital device/consumable frameworks)","competitors_carried":"Global medical-device brands; vascular access and catheters","priority":"🔴 5/5"},{"num":4,"name":"Allied Medical Group (AMG)","relevance":"Vascular/peripheral intervention + medical devices","contact":"☎️/email via website","website":"AMG Lebanon","latest_tender_date":"Jun 2026 (private-hospital interventional tenders)","competitors_carried":"International vascular/peripheral intervention brands; catheters","priority":"🔴 5/5"},{"num":5,"name":"Promedz Lebanon","relevance":"Interventional radiology + peripheral vascular + venous therapy","contact":"☎️ +961 70 827 807 · +961 1 364 659/60 · ✉️ promedz@promedz.com","website":"Promedz","latest_tender_date":"Jun 2026 (private-hospital interventional tenders)","competitors_carried":"Global IR and peripheral vascular brands; venous therapy","priority":"🔴 5/5"},{"num":6,"name":"Intelmed S.A.R.L.","relevance":"Vascular + interventional radiology + cardiothoracic surgery","contact":"☎️ +961 1 425 724","website":"—","latest_tender_date":"Jun 2026 (private-hospital interventional tenders)","competitors_carried":"International vascular/IR/cardiothoracic brands; catheters","priority":"🔴 5/5"},{"num":7,"name":"REMED Medical Equipment","relevance":"Trusted supplier of medical equipment/accessories/supplies in Beirut; dedicated sales/technical services","contact":"☎️ +961 70 701 696 / +961 3 124 790 · ✉️ info@remed-lb.com","website":"REMED Lebanon","latest_tender_date":"Jun 2026 (private-hospital consumable tenders)","competitors_carried":"International medical-equipment brands; catheters and disposables","priority":"🟠 4/5"},{"num":8,"name":"Biofield Medical","relevance":"Specialized medical consumables for interventional fields","contact":"Website contact","website":"Biofield Medical","latest_tender_date":"Jun 2026 (private-hospital consumable tenders)","competitors_carried":"Specialized interventional consumables; catheters","priority":"🟠 4/5"},{"num":9,"name":"Hayek Investment / HayekInv Medical","relevance":"Disposable devices for IR + vascular surgery","contact":"☎️ +961 1 87 33 81 · +961 3 66 22 72","website":"—","latest_tender_date":"Jun 2026 (private-hospital interventional tenders)","competitors_carried":"Disposable IR/vascular devices; multi-brand catheters","priority":"🟠 4/5"},{"num":10,"name":"Serum Product Co. Sarl","relevance":"Retailers of dialysis apparatus; long-standing presence in Metn/Beirut","contact":"☎️ +961 5 480 207 / +961 5 480 208","website":"Serum Product","latest_tender_date":"Jun 2026 (private-clinic dialysis procurements)","competitors_carried":"Dialysis apparatus and consumables; HD catheters","priority":"🟠 4/5"}],"iq":[{"num":1,"name":"SIDRAL S.A.R.L","relevance":"Fresenius Medical Care partner; operates 6 MOH-authorized dialysis centers; distributes dialysis machines/consumables nationwide","contact":"☎️ +964 780 377 0000 / +964 780 788 9918 · ✉️ info@sidral.com","website":"SIDRAL","latest_tender_date":"Sep 2026 (KIMADIA hemodialysis national-project tenders)","competitors_carried":"Fresenius Medical Care (dialysis systems & consumables); HD catheters","priority":"🔴 5/5"},{"num":2,"name":"Jadarah Scientific Bureau","relevance":"Leading medical-devices distributor since 2004; explicitly lists long-term & short-term hemodialysis catheters","contact":"☎️ +964 770 456 42 16 · ✉️ info@jadarah-iq.com","website":"Jadarah Iraq","latest_tender_date":"Sep 2026 (KIMADIA medical-supplies frameworks)","competitors_carried":"Multiple international HD catheter brands; dialysis consumables","priority":"🔴 5/5"},{"num":3,"name":"Al-Hayat Company (Hayat IQ)","relevance":"Laboratory & medical-appliance distributor; broad hospital-supply network; vascular-access consumables","contact":"☎️ +964 773 825 5919 · ✉️ info@hayatiq.com","website":"Hayat IQ","latest_tender_date":"Aug 2026 (MOH/private-hospital consumable tenders)","competitors_carried":"Multi-brand medical consumables; catheters and disposables","priority":"🔴 5/5"},{"num":4,"name":"Noor AlAdeeb Scientific Bureau","relevance":"MOH-approved medical-supplies distributor; serves hospitals, clinics, government institutions; certified disposables","contact":"Website/contact form","website":"Noor AlAdeeb","latest_tender_date":"Aug 2026 (MOH consumable frameworks)","competitors_carried":"International medical disposables; catheters","priority":"🔴 5/5"},{"num":5,"name":"KIMADIA (State Company for Marketing Drugs & Medical Appliances)","relevance":"Central MOH procurement entity; runs national hemodialysis and medical-appliance tenders","contact":"✉️ dg@kimadia.gov.iq · ☎️ +964 1 415 7667","website":"KIMADIA","latest_tender_date":"Sep 2026 (Hemodialysis National Project Service/Expansion tenders)","competitors_carried":"Multiple international HD catheter brands via tender awards","priority":"🔴 5/5"},{"num":6,"name":"Al-Rayan Medical","relevance":"Medical-equipment and consumables distributor; serves public/private hospitals","contact":"Contact via website","website":"—","latest_tender_date":"Aug 2026 (private-hospital consumable tenders)","competitors_carried":"Multi-brand medical equipment; catheters","priority":"🟠 4/5"},{"num":7,"name":"Zahraa Medical Supplies","relevance":"Surgical and medical consumables; hospital/clinic distribution","contact":"Contact via website","website":"—","latest_tender_date":"Jul 2026 (private-hospital consumable tenders)","competitors_carried":"International surgical consumables; catheters","priority":"🟠 4/5"},{"num":8,"name":"Tigris Medical","relevance":"Medical devices and equipment; broad consumables portfolio","contact":"Contact via website","website":"—","latest_tender_date":"Jul 2026 (private-hospital consumable tenders)","competitors_carried":"Global medical-device brands; catheters","priority":"🟠 4/5"},{"num":9,"name":"Mesopotamia Medical Trading","relevance":"Medical-equipment trading; hospital-supply networks","contact":"Contact via website","website":"—","latest_tender_date":"Jun 2026 (private-sector procurements)","competitors_carried":"Multi-brand medical consumables; catheters","priority":"🟡 3/5"},{"num":10,"name":"Baghdad Medical Supplies","relevance":"General medical supplies; serves clinics and smaller hospitals","contact":"Contact via website","website":"—","latest_tender_date":"Jun 2026 (private-clinic consumable tenders)","competitors_carried":"International medical supplies; catheters","priority":"🟡 3/5"}]},"kols":{"sa":[{"num":1,"name":"Prof. Faissal A. M. Shaheen","relevance":null,"specialty":"⭐⭐⭐⭐⭐ Nephrology / transplantation","institution":"Dr. Soliman Fakeeh Hospital / SCOT","contact":"✉️ famshaheen@gmail.com","extra":null,"extra_label":""},{"num":2,"name":"Prof. Abdullah Al-Hwiesh","relevance":null,"specialty":"⭐⭐⭐⭐⭐ Nephrology / dialysis / vascular access","institution":"King Fahd Hospital of the University / IAU","contact":"✉️ ahwiesh@iau.edu.sa","extra":null,"extra_label":""},{"num":3,"name":"Dr. Abdullah Al Sayyari","relevance":null,"specialty":"⭐⭐⭐⭐⭐ Nephrology / dialysis","institution":"MNGHA / King Abdulaziz Medical City","contact":"MNGHA Nephrology Department","extra":null,"extra_label":""},{"num":4,"name":"Dr. Ali Alharbi","relevance":null,"specialty":"⭐⭐⭐⭐⭐ Nephrology / dialysis","institution":"Diaverum Saudi Arabia","contact":"Professional profile","extra":null,"extra_label":""},{"num":5,"name":"Dr. Dujanah Hassan Mousa","relevance":null,"specialty":"⭐⭐⭐⭐ Nephrology / dialysis","institution":"Diaverum Saudi Arabia","contact":"Diaverum Saudi Arabia","extra":null,"extra_label":""},{"num":6,"name":"Dr. Mohammed Alhomrany","relevance":null,"specialty":"⭐⭐⭐⭐ Nephrology / dialysis","institution":"Diaverum Saudi Arabia","contact":"Diaverum Saudi Arabia","extra":null,"extra_label":""},{"num":7,"name":"Dr. Fayez Alhejaili","relevance":null,"specialty":"⭐⭐⭐⭐ Nephrology / dialysis","institution":"Diaverum Saudi Arabia","contact":"Diaverum Saudi Arabia","extra":null,"extra_label":""},{"num":8,"name":"Dr. Hassan Alshehri","relevance":null,"specialty":"⭐⭐⭐⭐ Interventional Radiology","institution":"Prince Sultan Military Medical City, Riyadh","contact":"Saudi Interventional Radiology Society","extra":null,"extra_label":""},{"num":9,"name":"Dr. Shaker Alshehri","relevance":null,"specialty":"⭐⭐⭐⭐ Vascular & Interventional Radiology","institution":"King Abdulaziz Medical City, Riyadh","contact":"Saudi Interventional Radiology Society","extra":null,"extra_label":""},{"num":10,"name":"Dr. Shagran Binkhamis","relevance":null,"specialty":"⭐⭐⭐⭐ Vascular & Interventional Radiology","institution":"King Faisal Specialist Hospital & Research Centre","contact":"Saudi Interventional Radiology Society","extra":null,"extra_label":""}],"ae":[{"num":1,"name":"Dr. Ayman Kamal Almadani","relevance":null,"specialty":"Nephrology / dialysis leadership","institution":"SEHA Kidney Care","contact":"SEHA appointment/contact","extra":null,"extra_label":""},{"num":2,"name":"Dr. Wasim Ahmed","relevance":null,"specialty":"Nephrology / advanced HD","institution":"SEHA Kidney Care","contact":"SEHA appointment/contact","extra":null,"extra_label":""},{"num":3,"name":"Dr. Salaheldin Khalil Issa","relevance":null,"specialty":"Nephrology / advanced HD","institution":"SEHA Kidney Care","contact":"SEHA appointment/contact","extra":null,"extra_label":""},{"num":4,"name":"Dr. Hormaz Dara Dastoor","relevance":null,"specialty":"Nephrology / advanced HD","institution":"SEHA Kidney Care","contact":"SEHA appointment/contact","extra":null,"extra_label":""},{"num":5,"name":"Dr. Mohammad Raafat Al Hakim","relevance":null,"specialty":"Nephrology / dialysis / RRT","institution":"SEHA Kidney Care – Al Ain / Tawam","contact":"SEHA appointment/contact","extra":null,"extra_label":""},{"num":6,"name":"Dr. Anvar Hussain Hamid Khan","relevance":null,"specialty":"Nephrology / vascular disease / HD","institution":"SEHA Kidney Care","contact":"☎️ 80050 / SEHA appointment","extra":null,"extra_label":""},{"num":7,"name":"Dr. Mohamed Hassan","relevance":null,"specialty":"Nephrology / HD / PD / transplant","institution":"SEHA Kidney Care","contact":"SEHA appointment/contact","extra":null,"extra_label":""},{"num":8,"name":"Dr. Abraham George","relevance":null,"specialty":"Nephrology / HD / vascular disease","institution":"SEHA Kidney Care – Al Ain","contact":"SEHA appointment/contact","extra":null,"extra_label":""},{"num":9,"name":"Dr. Hefsa Al Shamsi","relevance":null,"specialty":"Nephrology / HD / transplantation","institution":"SEHA Kidney Care","contact":"SEHA appointment/contact","extra":null,"extra_label":""},{"num":10,"name":"Dr. Fadi Hijazi","relevance":null,"specialty":"Nephrology","institution":"Cleveland Clinic Abu Dhabi","contact":"Cleveland Clinic Abu Dhabi appointment","extra":null,"extra_label":""}],"qa":[{"num":1,"name":"Dr. Hassan Al-Malki","relevance":null,"specialty":"⭐ Nephrology / dialysis leadership","institution":"HMC","contact":"HMC +974 4439 5777","extra":"🔴 5/5","extra_label":"Priority"},{"num":2,"name":"Dr. Omar Fituri","relevance":null,"specialty":"⭐ Nephrology / transplant / RRT","institution":"HMC + Weill Cornell Medicine-Qatar","contact":"HMC / WCM-Q","extra":"🔴 5/5","extra_label":"Priority"},{"num":3,"name":"Dr. Muhammad Asim","relevance":null,"specialty":"⭐ Senior nephrology / dialysis / CRRT","institution":"HMC","contact":"HMC +974 4439 5777","extra":"🔴 5/5","extra_label":"Priority"},{"num":4,"name":"Dr. Ihab T. M. Elmadhoun","relevance":null,"specialty":"⭐ Nephrology / CRRT / dialysis","institution":"HMC","contact":"HMC +974 4439 5777","extra":"🔴 5/5","extra_label":"Priority"},{"num":5,"name":"Dr. Abdullah Ibrahim Hamad","relevance":null,"specialty":"⭐ Nephrology / dialysis","institution":"HMC","contact":"HMC +974 4439 5777","extra":"🔴 5/5","extra_label":"Priority"},{"num":6,"name":"Dr. Muftah Othman","relevance":null,"specialty":"⭐ Senior nephrology / dialysis","institution":"HMC","contact":"HMC +974 4439 5777","extra":"🔴 5/5","extra_label":"Priority"},{"num":7,"name":"Dr. Khaled Mahmoud","relevance":null,"specialty":"⭐ Nephrology / dialysis","institution":"HMC","contact":"HMC +974 4439 5777","extra":"🔴 5/5","extra_label":"Priority"},{"num":8,"name":"Dr. Alaedine Shurrab","relevance":null,"specialty":"⭐ Nephrology / renal replacement therapy","institution":"HMC / Al Khor Hospital","contact":"HMC +974 4439 5777","extra":"🟠 4/5","extra_label":"Priority"},{"num":9,"name":"Dr. Awais Nauman","relevance":null,"specialty":"Nephrology / renal medicine","institution":"HMC","contact":"HMC +974 4439 5777","extra":"🟠 4/5","extra_label":"Priority"},{"num":10,"name":"Dr. Ali A. Haydar","relevance":null,"specialty":"⭐ Interventional radiology / vascular intervention","institution":"Aman Hospital","contact":"☎️ +974 4400 4400 · ✉️ [email protected]","extra":"🔴 5/5","extra_label":"Priority"}],"kw":[{"num":1,"name":"Prof. Hamed Al-Essa","relevance":null,"specialty":"⭐ Nephrology / transplant / dialysis","institution":"Kuwait renal network","contact":"MOH / hospital","extra":null,"extra_label":"Priority"},{"num":2,"name":"Dr. Hamad Behbehani","relevance":null,"specialty":"⭐ Nephrology / renal medicine","institution":"Kuwait MOH","contact":"MOH / hospital","extra":null,"extra_label":"Priority"},{"num":3,"name":"Dr. Omar Al-Hunidi","relevance":null,"specialty":"⭐ Nephrology / renal medicine","institution":"Kuwait","contact":"Hospital / clinic","extra":null,"extra_label":"Priority"},{"num":4,"name":"Dr. Ahmed Ramadan","relevance":null,"specialty":"Nephrology / renal medicine","institution":"Amiri Hospital","contact":"MOH / Amiri","extra":null,"extra_label":"Priority"},{"num":5,"name":"Dr. Hisham Al-Sabah","relevance":null,"specialty":"Nephrology / renal medicine","institution":"Kuwait MOH","contact":"MOH","extra":null,"extra_label":"Priority"},{"num":6,"name":"Dr. Abdulaziz Al-Mousawi","relevance":null,"specialty":"Nephrology / dialysis","institution":"Kuwait MOH","contact":"MOH / hospital","extra":null,"extra_label":"Priority"},{"num":7,"name":"Dr. Mohammed Al-Mousawi","relevance":null,"specialty":"Nephrology / renal medicine","institution":"Kuwait","contact":"MOH / hospital","extra":null,"extra_label":"Priority"},{"num":8,"name":"Dr. Khaled Al-Sabah","relevance":null,"specialty":"Nephrology / renal medicine","institution":"Kuwait","contact":"MOH / hospital","extra":null,"extra_label":"Priority"},{"num":9,"name":"Dr. Faisal Al-Rashidi","relevance":null,"specialty":"Nephrology / dialysis","institution":"Kuwait","contact":"MOH / hospital","extra":null,"extra_label":"Priority"},{"num":10,"name":"Dr. Ahmed Al-Sabah","relevance":null,"specialty":"Renal medicine / transplantation","institution":"Kuwait","contact":"MOH / hospital","extra":null,"extra_label":"Priority"}],"om":[{"num":1,"name":"Dr. Dawood Al-Riyami","relevance":null,"specialty":"⭐ Nephrology / dialysis","institution":"Sultan Qaboos University Hospital","contact":"✉️ dawood@squ.edu.om","extra":"🔴 5/5","extra_label":"Priority"},{"num":2,"name":"Dr. Ali Al Lawati","relevance":null,"specialty":"⭐ Nephrology / dialysis","institution":"Sultan Qaboos University Hospital","contact":"✉️ aallawati@squ.edu.om","extra":"🔴 5/5","extra_label":"Priority"},{"num":3,"name":"Dr. Sadiq Al Lawati","relevance":null,"specialty":"⭐ Senior Consultant Nephrologist","institution":"Royal Hospital","contact":"Royal Hospital / MOH","extra":"🔴 5/5","extra_label":"Priority"},{"num":4,"name":"Dr. Issa Al Salmi","relevance":null,"specialty":"⭐ Senior Consultant Nephrologist","institution":"Royal Hospital","contact":"Royal Hospital / MOH","extra":"🔴 5/5","extra_label":"Priority"},{"num":5,"name":"Dr. Alan Hola","relevance":null,"specialty":"⭐ Senior Consultant Nephrologist","institution":"Royal Hospital","contact":"Royal Hospital / MOH","extra":"🔴 5/5","extra_label":"Priority"},{"num":6,"name":"Dr. Mahmood Nasser Al Hajiry","relevance":null,"specialty":"⭐ IR / dialysis access / PermCath / PD catheter","institution":"Royal Hospital","contact":"Aster / Royal Hospital","extra":"🔴 5/5","extra_label":"Priority"},{"num":7,"name":"Dr. Tamer Sayed Fouad","relevance":null,"specialty":"⭐ Vascular & endovascular surgery / HD access","institution":"Burjeel Hospital Oman","contact":"Burjeel Hospital","extra":"🔴 5/5","extra_label":"Priority"},{"num":8,"name":"Dr. Said Al-Lamki","relevance":null,"specialty":"⭐ Interventional Radiology / central venous catheter insertion","institution":"Burjeel Hospital Muscat","contact":"Burjeel Hospital","extra":"🔴 5/5","extra_label":"Priority"},{"num":9,"name":"Dr. Faisal Al Balushi","relevance":null,"specialty":"Interventional Radiology","institution":"Royal Hospital","contact":"Oman Vascular Society / Royal Hospital","extra":"🟠 4/5","extra_label":"Priority"},{"num":10,"name":"Dr. Suliman Al Shamsi","relevance":null,"specialty":"⭐ Senior Consultant Vascular Surgeon","institution":"Royal Hospital","contact":"Royal Hospital / MOH","extra":"🔴 5/5","extra_label":"Priority"}],"jo":[{"num":1,"name":"Prof. Riyad Abdel Raouf Saeed","relevance":null,"specialty":"⭐ Nephrology / kidney transplantation","institution":"Jordan Hospital","contact":"☎️ +962 6 560 8080","extra":"🔴 5/5","extra_label":"Priority"},{"num":2,"name":"Dr. Fouad Riad Saeed","relevance":null,"specialty":"⭐ Nephrology / transplantation","institution":"Jordan Hospital","contact":"☎️ +962 6 560 8080 · ✉️ info@jordan-hospital.com","extra":"🔴 5/5","extra_label":"Priority"},{"num":3,"name":"Dr. Bisher Kawar","relevance":null,"specialty":"⭐ Nephrology / dialysis / transplantation","institution":"Abdali Hospital","contact":"☎️ +962 6 510 9999","extra":"🔴 5/5","extra_label":"Priority"},{"num":4,"name":"Dr. Hiba Barghouthi","relevance":null,"specialty":"⭐ Nephrology","institution":"Abdali Hospital","contact":"☎️ +962 6 510 9999","extra":"🔴 5/5","extra_label":"Priority"},{"num":5,"name":"Dr. Jawad Syouri","relevance":null,"specialty":"⭐ Nephrology / kidney transplant","institution":"Ibn Al-Haytham Hospital","contact":"Hospital: +962 6 569 4420","extra":"🔴 5/5","extra_label":"Priority"},{"num":6,"name":"Dr. Ahmed Rashid","relevance":null,"specialty":"⭐ Nephrology / internal medicine","institution":"Al Khalidi Hospital","contact":"☎️ +962 6 464 4281","extra":"🔴 5/5","extra_label":"Priority"},{"num":7,"name":"Dr. Bashar Zuhair Ghosheh","relevance":null,"specialty":"⭐ Vascular Surgery","institution":"Jordan Hospital","contact":"☎️ +962 6 560 8080","extra":"🔴 5/5","extra_label":"Priority"},{"num":8,"name":"Dr. Omar Nader Hamdallah","relevance":null,"specialty":"⭐ Vascular surgery + catheterization + kidney transplant","institution":"Jordan Hospital / Jordan Vascular Clinic","contact":"☎️ Hospital +962 6 560 8080","extra":"🔴 5/5","extra_label":"Priority"},{"num":9,"name":"Dr. Sizeph Haddad","relevance":null,"specialty":"⭐ Vascular & Interventional Radiology","institution":"Abdali Hospital","contact":"☎️ +962 6 510 9999","extra":"🔴 5/5","extra_label":"Priority"},{"num":10,"name":"Dr. Farid Al-Adham","relevance":null,"specialty":"⭐ Interventional radiology / vascular catheter procedures","institution":"Amman","contact":"Vezeeta / clinic","extra":"🟠 4/5","extra_label":"Priority"}],"lb":[{"num":1,"name":"Dr. Hicham Cheikh Hassan","relevance":null,"specialty":"⭐ Nephrology / dialysis / renal vascular services","institution":"LAU Medical Center","contact":"LAU Medicine","extra":"🔴 5/5","extra_label":"Priority"},{"num":2,"name":"Prof. Dania Chelala","relevance":null,"specialty":"⭐ Nephrology / HD / transplantation","institution":"Hôtel-Dieu de France","contact":"HDF","extra":"🔴 5/5","extra_label":"Priority"},{"num":3,"name":"Dr. Hiba Azar","relevance":null,"specialty":"Nephrology / dialysis","institution":"Hôtel-Dieu de France","contact":"HDF","extra":"🔴 5/5","extra_label":"Priority"},{"num":4,"name":"Dr. Kassem Bdeiri","relevance":null,"specialty":"Nephrology / dialysis","institution":"Hôtel-Dieu de France","contact":"HDF","extra":"🟠 4/5","extra_label":"Priority"},{"num":5,"name":"Dr. Majdi Hamedeh","relevance":null,"specialty":"⭐ Nephrology + dialysis","institution":"Al Zahraa Hospital UMC","contact":"✉️ majdi.hmedeh@zhumc.org.lb · ☎️ +961 1 851040","extra":"🔴 5/5","extra_label":"Priority"},{"num":6,"name":"Dr. Lynn Bou Khalil","relevance":null,"specialty":"⭐ Nephrology & Hypertension","institution":"Mount Lebanon Hospital UMC","contact":"☎️ +961 25 957 000","extra":"🔴 5/5","extra_label":"Priority"},{"num":7,"name":"Prof. Jamal Hoballah","relevance":null,"specialty":"⭐ Vascular surgery","institution":"AUB Medical Center","contact":"AUB","extra":"🔴 5/5","extra_label":"Priority"},{"num":8,"name":"Dr. Fady Haddad","relevance":null,"specialty":"⭐ Vascular Surgery","institution":"Mount Lebanon Hospital UMC","contact":"☎️ +961 25 957 000","extra":"🔴 5/5","extra_label":"Priority"},{"num":9,"name":"Dr. Abdallah Noufaily","relevance":null,"specialty":"⭐ Interventional vascular/nonvascular radiology","institution":"LAU Medical Center","contact":"☎️ +961 1 200800 ext. 6979","extra":"🔴 5/5","extra_label":"Priority"},{"num":10,"name":"Dr. Hadi Khoury","relevance":null,"specialty":"⭐ Interventional Radiology / vascular intervention","institution":"Khoury Vascular Clinic","contact":"KVC","extra":"🟠 4/5","extra_label":"Priority"}],"iq":[{"num":1,"name":"Prof. Arif Sami Malik","relevance":null,"specialty":"⭐ Nephrology / HD + PD","institution":"Al-Nahrain University / Iraq","contact":"✉️ dr.arifsami@nahrainuniv.edu.iq","extra":"🔴 5/5","extra_label":"Priority"},{"num":2,"name":"Dr. Zaid Ali","relevance":null,"specialty":"⭐ Vascular surgery / angiography / angioplasty","institution":"Ministry of Health, Al-Muthanna","contact":"PAIRS physician directory","extra":"🔴 5/5","extra_label":"Priority"},{"num":3,"name":"Dr. Fadhil Al-Ammar","relevance":null,"specialty":"Medical/academic leadership","institution":"Founder, Nova Scientific Bureau","contact":"Nova Scientific Bureau","extra":"🟠 4/5","extra_label":"Priority"},{"num":4,"name":"Dr. Abdul-Hadi Al-Hassan","relevance":null,"specialty":"Nephrology / renal medicine","institution":"Iraqi nephrology network","contact":"Hospital/professional route","extra":"🔴 5/5","extra_label":"Priority"},{"num":5,"name":"Dr. Ahmed Al-Jubouri","relevance":null,"specialty":"Nephrology / dialysis","institution":"Iraqi renal-care network","contact":"Hospital/professional route","extra":"🔴 5/5","extra_label":"Priority"},{"num":6,"name":"Dr. Ali Al-Mashhadani","relevance":null,"specialty":"Nephrology / dialysis","institution":"Baghdad","contact":"Hospital/professional route","extra":"🟠 4/5","extra_label":"Priority"},{"num":7,"name":"Dr. Raad Al-Khafaji","relevance":null,"specialty":"Vascular surgery","institution":"Baghdad / MOH","contact":"Hospital/professional route","extra":"🔴 5/5","extra_label":"Priority"},{"num":8,"name":"Dr. Haider Al-Saadi","relevance":null,"specialty":"Interventional radiology","institution":"Baghdad","contact":"Hospital/professional route","extra":"🔴 5/5","extra_label":"Priority"},{"num":9,"name":"Dr. Mohammed Al-Taie","relevance":null,"specialty":"Interventional radiology / vascular intervention","institution":"Baghdad","contact":"Hospital/professional route","extra":"🔴 5/5","extra_label":"Priority"},{"num":10,"name":"Dr. Ahmed Al-Bayati","relevance":null,"specialty":"Vascular / endovascular surgery","institution":"Iraq","contact":"Hospital/professional route","extra":"🟠 4/5","extra_label":"Priority"}],"bh":[{"num":1,"name":"Dr. Issa Kawalit","relevance":null,"specialty":"⭐ Nephrology + dialysis + transplant","institution":"Royal Bahrain Hospital","contact":"☎️ +973 1724 6800","extra":"🔴 5/5","extra_label":"Priority"},{"num":2,"name":"Dr. Abdulraqeeb Alomari","relevance":null,"specialty":"⭐ Nephrologist + kidney transplant","institution":"Royal Bahrain Hospital","contact":"☎️ +973 1724 6800","extra":"🔴 5/5","extra_label":"Priority"},{"num":3,"name":"Dr. Muhand Salemah Raji Eltwal","relevance":null,"specialty":"⭐ Nephrology","institution":"Royal Bahrain Hospital","contact":"☎️ +973 1724 6800","extra":"🔴 5/5","extra_label":"Priority"},{"num":4,"name":"Dr. Ahmed Mordi","relevance":null,"specialty":"⭐ Interventional Radiology + dialysis access","institution":"Royal Bahrain Hospital","contact":"☎️ +973 1724 6800 · WhatsApp +973 3218 1810","extra":"🔴 5/5","extra_label":"Priority"},{"num":5,"name":"Dr. Wadie Yousif","relevance":null,"specialty":"⭐ Vascular & Interventional Radiology","institution":"Ibn Al-Nafees Hospital","contact":"☎️ +973 1782 8282 / 1782 8253","extra":"🔴 5/5","extra_label":"Priority"},{"num":6,"name":"Dr. Sharif Abdulsalam Hamza Khashaba","relevance":null,"specialty":"⭐ Vascular Surgery","institution":"Royal Bahrain Hospital","contact":"☎️ +973 1724 6800","extra":"🔴 5/5","extra_label":"Priority"},{"num":7,"name":"Dr. Sawsan Kadhem","relevance":null,"specialty":"Interventional Radiology","institution":"Dawali Clinics / Salmaniya","contact":"Hospital/clinic route","extra":"🟠 4/5","extra_label":"Priority"},{"num":8,"name":"Dr. Jinane Khaled","relevance":null,"specialty":"Radiology","institution":"Royal Bahrain Hospital","contact":"☎️ +973 1724 6800","extra":"🟠 4/5","extra_label":"Priority"},{"num":9,"name":"Dr. Suzanne Abbas","relevance":null,"specialty":"Radiology","institution":"Royal Bahrain Hospital","contact":"☎️ +973 1724 6800","extra":"🟠 4/5","extra_label":"Priority"},{"num":10,"name":"Dr. Fatema Abdulrahman","relevance":null,"specialty":"Radiology","institution":"Royal Bahrain Hospital","contact":"☎️ +973 1724 6800","extra":"🟠 4/5","extra_label":"Priority"}]}}
...
function openNetwork(type,code){
  const meta=networkCountryMeta[code],d=countryData[code],page=document.getElementById('page-'+type+'-'+code);if(!meta||!d||!page)return;
  document.querySelectorAll('.page').forEach(p=>p.classList.remove('active'));document.querySelectorAll('.nav-item').forEach(n=>n.classList.remove('active'));page.classList.add('active');
  page.style.setProperty('--net-primary',d.colors.primary);page.style.setProperty('--net-accent',d.colors.accent);page.style.setProperty('--net-secondary',d.colors.secondary);
  const rows=(networkData[type]&&networkData[type][code])||[],isDist=type==='distributors',label=isDist?'Distributors':'KOLs',icon=isDist?'🤝':'⭐';
  const esc=v=>String(v??'—').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');
  page.innerHTML=`<div class="network-shell"><div class="network-hero"><div class="network-hero-top"><button class="network-back" onclick="openCountry('${code}')">← Country Analysis</button><img class="network-flag" src="${d.flagImg}" alt="${meta.name} flag" onerror="this.style.display='none'"><div><div class="network-title">${icon} ${meta.name} — ${label}</div><div class="network-subtitle">Workbook-driven ${label.toLowerCase()} directory</div></div><div class="network-theme-chip"><span class="network-theme-dot"></span>${meta.name} Theme</div></div></div><div class="network-toolbar"><div><div class="network-section-title">${icon} ${label} Directory</div><div class="network-section-sub">All fields are sourced directly from the workbook.</div></div><div style="font-size:9px;color:#607a9f">${rows.length} records</div></div><div class="network-table-wrap"><div class="network-table-scroll"><table class="network-table" aria-label="${meta.name} ${label} directory"><thead><tr>${isDist?'<th scope="col">#</th><th scope="col">Distributor</th><th scope="col">Why Relevant</th><th scope="col">Contact</th><th scope="col">Website</th><th scope="col">Latest Tender Date</th><th scope="col">Competitors / Suppliers Carried</th><th scope="col">Priority</th>':'<th scope="col">#</th><th scope="col">KOL</th><th scope="col">Specialty</th><th scope="col">Institution</th><th scope="col">Priority</th><th scope="col">Contact</th>'}</tr></thead><tbody>${rows.map(r=>isDist?`<tr><td class="network-table-num">${esc(r.num)}</td><td><div class="network-table-name">${esc(r.name)}</div></td><td><div class="network-table-main">${esc(r.relevance)}</div></td><td><div class="network-contact-cell">${esc(r.contact)}</div></td><td><div class="network-contact-cell">${esc(r.website||'Contact via phone/email')}</div></td><td><div class="network-contact-cell">${esc(r.latest_tender_date)}</div></td><td><div class="network-contact-cell">${esc(r.competitors_carried)}</div></td><td><span class="network-priority ${networkPriorityClass(r.priority)}">${esc(r.priority)}</span></td></tr>`:`<tr><td class="network-table-num">${esc(r.num)}</td><td><div class="network-table-name">${esc(r.name)}</div></td><td><div class="network-table-main">${esc(r.specialty)}</div></td><td><div class="network-table-main">${esc(r.institution)}</div></td><td><span class="network-priority ${networkPriorityClass(r.extra)}">${esc(r.extra||'—')}</span></td><td><div class="network-contact-cell">${esc(r.contact)}</div></td></tr>`).join('')||`<tr><td colspan="${isDist?8:6}" class="network-empty">No records available.</td></tr>`}</tbody></table></div></div><div class="network-footer">Source: ${isDist?'Distributors':'KOL_Catalog'} sheet</div></div>`;
}
...
function renderCompetitorShareChart_DISABLED(){
...
(function () {
  ...
  (function updateTenderKpis(){
    var total = tndrData.length;
    var critical = tndrData.filter(function(r){ return String(r.priority||'').toLowerCase()==='critical'; }).length;
    var active = tndrData.filter(function(r){ var x=String(r.status||'').toLowerCase(); return x==='open' || x==='active'; }).length;
    var closed = tndrData.filter(function(r){ return String(r.status||'').toLowerCase()==='closed'; }).length;
    var set=function(id,v){var e=document.getElementById(id);if(e)e.textContent=v;};
    set('tndr-kpi-total',total); set('tndr-kpi-critical',critical); set('tndr-kpi-active',active); set('tndr-kpi-closed',closed);
  })();
...
(function(){const wd=workbookData,esc=v=>String(v??'—').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');
const rg=document.getElementById('reg-country-grid');if(rg)Object.entries(wd.regulatory||{}).forEach(([code,d])=>{const card=document.createElement('div');card.className='reg-country-card';card.innerHTML=`<div class="reg-country-head" role="button" tabindex="0" aria-expanded="false"><strong>${esc(d.country)}</strong><div class="reg-country-meta">${esc(d.authority||'Workbook regulatory detail')}</div></div><div class="reg-country-body">${(d.items||[]).map(it=>`<div class="reg-item"><div class="reg-item-title">${esc(it.item)}</div><div class="reg-item-details"><ul>${(it.details||[]).map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div></div>`).join('')}</div>`;const h=card.querySelector('.reg-country-head');const t=()=>{const o=card.classList.toggle('open');h.setAttribute('aria-expanded',o?'true':'false')};h.onclick=t;h.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();t()}};rg.appendChild(card)});
const pg=document.getElementById('procurement-grid');if(pg)(wd.procurement||[]).forEach(p=>{const ts=(wd.tenders||[]).filter(t=>t.country_code===p.country_code||t.country===p.country);const card=document.createElement('div');card.className='proc-country-card';card.innerHTML=`<div class="proc-country-head"><strong>${esc(p.country)}</strong><div class="reg-country-meta">${ts.length} linked tender(s)</div></div><div class="proc-country-body"><div class="proc-label">Primary Procurement Body</div><div class="proc-value">${esc(p.primary_body)}</div><div class="proc-label">Secondary / Parallel Buyers</div><div class="proc-value">${esc(p.secondary_buyers)}</div><div class="proc-label">Notes</div><div class="proc-value">${esc(p.notes)}</div><div class="proc-tender-wrap"><table class="proc-mini-table" aria-label="${esc(p.country)} tenders"><thead><tr><th>Tender</th><th>Deadline</th><th>Status</th><th>Priority</th></tr></thead><tbody>${ts.map(t=>`<tr><td>${esc(t.name)}</td><td>${esc(t.deadline)}</td><td>${esc(t.status)}</td><td>${esc(t.priority)}</td></tr>`).join('')||'<tr><td colspan="4">No linked tender rows.</td></tr>'}</tbody></table></div></div>`;pg.appendChild(card)});
const fp=document.getElementById('page-forecast');if(fp&&wd.forecast){const b=document.createElement('div');b.style.cssText='margin-top:16px;background:#0f1f3d;border:1px solid #1e3d7a;border-radius:14px;padding:18px';b.innerHTML=`<div style="font-size:13px;font-weight:700;color:#c8d8f0">Forecast Data Detail</div><div style="font-size:10px;color:#6a85b0;margin-top:4px">Product mix: STD ${(wd.forecast.product_mix.std*100).toFixed(0)}% · Mid-Term ${(wd.forecast.product_mix.mid_term*100).toFixed(0)}% · Tunneled ${(wd.forecast.product_mix.tunneled*100).toFixed(0)}%</div><select id="fd-country" style="margin-top:12px;background:#081321;color:#e8edf5;border:1px solid #1e3d7a;border-radius:8px;padding:8px">${wd.forecast.countries.map(c=>`<option value="${c.country_code}">${c.country}</option>`).join('')}</select><div id="fd-detail" style="margin-top:12px"></div><div style="margin-top:12px;color:#8fa8cf;font-size:10px;line-height:1.6">${wd.forecast.methodology.map(x=>`• ${esc(x)}<br>`).join('')}</div>`;fp.appendChild(b);const render=()=>{const c=wd.forecast.countries.find(x=>x.country_code===document.getElementById('fd-country').value);document.getElementById('fd-detail').innerHTML=c?`2026 Demand <b>${Number(c.demand_2026).toLocaleString()}</b> · Blended ASP <b>$${Number(c.blended_asp).toFixed(1)}</b> · 2026 Revenue <b>$${Number(c.revenue_2026).toLocaleString()}</b> · 2028 Revenue <b>$${Number(c.revenue_2028).toLocaleString()}</b>`:''};document.getElementById('fd-country').onchange=render;render()}
})();
/* ─── END SOURCES ─── */
</script>
</body>
